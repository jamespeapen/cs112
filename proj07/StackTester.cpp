...
void StackTester::testGetCapacityGetSize() {
	cout << "- Testing getCapacity() and getSize()..." << flush;
	Stack<double> s1(5);
...
void StackTester::testSetCapacity() {
	...
	Stack<double> s1(5), s2(4), s3(2);
	assert( s1.getCapacity() == 5 );
	cout << " 0 " << flush;
	///set 0 capacity
	try {
		s1.setCapacity(0);
		cerr << "Capacity set to 0";
		exit(1);
	} catch (const StackException& se) {
		cout << " 1 " << flush;
	}
	...
	try {
		s2.setCapacity(2);
		cerr << "Capacity set to less than mySize";
		exit(1);
	} catch (const StackException& se) {
		cout << " 2 " << flush;
	}
	//set higher capacity with data
	s3.push(11);
	s3.push(22);
	s3.setCapacity(5);
